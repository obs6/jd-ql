...
================Loon==============
[Script]
cron "37 22,23 * * *" script-path=jd_speed_redpocke.js,tag=我的京东极速版发财挖宝提现
...
function getRewordList() {
    return new Promise(resolve => {
        const body ={"linkId":linkId发财挖宝,"pageNum":1,"pageSize":10};
        $.get(taskGetUrl("spring_reward_list",body), async (err, resp, data) => {
            try {
                if (err) {
                        ...
                        if (data.code === 0) {
                            for(let item of data.data.items){
                                if(item.prizeType===4 && item.state===0){
                                    //{"id":8151608,"business":"happyDigger","poolBaseId":682,"prizeGroupId":1002,"prizeBaseId":1145,"prizeType":4},"linkId":"pTTvJeSTrpthgk9ASBVGsw"}
                                    cashInfo={
                                        "id":item.id,
                                        ...
                                        "prizeType":4};
                                    console.log(`去提现，金额=======${item.amount}`);
                                    await do提现(cashInfo);
...
function taskPostUrl(function_id, body) {
    return {
        ...
        body: `appid=activities_platform&functionId=${function_id}&body=${escape(JSON.stringify(body))}&t=${+new Date()}&client=H5&clientVersion=1.0.0`,
        headers: {
            'Cookie': cookie,
            'Host': 'api.m.jd.com',
            'Accept': '*/*',
